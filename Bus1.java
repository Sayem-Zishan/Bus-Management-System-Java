...
class Bus1 extends JFrame implements ActionListener
{
    int[] arr = new int[32];
    ...
    public void actionPerformed(ActionEvent a)
    {
        ...
        for(i=0;i<32;i++)
        {
            if (a.getSource() == jb[i])
            {
				...
				try{
					String query = "SELECT * FROM `bus_1` WHERE Seat_no="+'"'+str[i]+'"'+"AND Info='YES'";
					...
					if(count<1)
					{
						new payment(un,str[i]); //new payment(em,str[i]);
  ...
  /*  public void GetInfo()
    {
        ...
        try
        {
            String host = "jdbc:derby://localhost:1527/BusManagement";
            String uName = "a";
            String uPass = "a";
